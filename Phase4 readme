...
|---|---|---|
| Kevin Pol | Kevin_Pol@student.uml.edu | Sender implementation, experiments, plotting |
| Andrew Thach | Andrew_Thach@student.uml.edu | Receiver implementation, packet handling |
| Odey Khello | Odey_Khello@student.uml.edu | Fault injection, testing, validation |
...
### Start Receiver
```bash
python server_gbn_phase4.py 12000 1 0.0 received.bin
```

### Run Sender
```bash
python client_gbn_phase4.py 127.0.0.1 12000 1 0.0 5 0.10 send.bin
```
...
### Scenario 1: No loss / no errors (Option 1)
Receiver:
```bash
...python server_gbn_phase4.py 12000 1 0.0 received.bin
```
...
Reciever
...python server_gbn_phase4.py 12000 2 0.20 received.bin
Sender
---python client_gbn_phase4.py 127.0.0.1 12000 2 0.20 5 0.10 send.bin
...
- Higher loss rates make the transfer slower  
- Timeout is fixed 
- Didnt have enough time to finish the demo video  
- Plot was generated, but not many runs were done due to time  
